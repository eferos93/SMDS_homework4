...
```{r}
names <- c("Seven Sevens", "Annalong Horseshoe", "Slieve Donard", "Meelbeg Meelmore", "Flagstaff to Carling")
...
coef(y.lm)
```
Comparing this result obtained using `x2 <- rbinom(10, 1, x1)` with the one obtained using `x2 <- rbinom(10, 1, 1-x1)` in the model with only $x_2$ predictor, we can see that the coefficient of $x_2$ has now a positive sign. Moreover, we notice that the coefficient of $x_2$ is much larger than the expected value that is equal to $1$. This is due to the fact that $x_2$ also contains the information of $x_1$ since they are dependent.
```{r, echo=TRUE}
...
Regarding to the goodness of the predictions obtained with the estimated model, we can observe that, when $x$ tends to infinity (which corresponds to future predictions) and assuming that the estimated coefficients are such that $\hat a>0$ and $\hat b <0$, the rate $\lambda$ of the Poisson distribution tends to $0$ and also its variance, since $\lambda=e^{\hat a+ \hat b x}$. For this reason, regardless of the values of the estimated coefficients, the future predicitons will be really close to the true values since the variance $var(\lambda)$ tends to $0$ as $x$ tends to $+\infty$.  

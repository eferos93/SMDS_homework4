...
The result of the $F$ test give raise to a p-value, which is the probability that the $F$ statistics is grater than the one observed assuming the null hypotesis relative to `dist x climb` , is quite low. Thus we can reject the null hypotesis and therefore say that `nihills2.lm`, the most complex model that includes an interaction term, appears significantly better than Model 1 for `nihills` data. 
...
Looking to these plots of the two fit at first istance we observe that `Seven Sevens` observation looks to be a suspect point. This point has an unexpected residual and looks to have a large influence to the fit.
In fact, the `Residual vs Leverage` plot suggest that this point has a very large leverage and, at the same time, a high standardized residual. We think then `Seven Sevens` is clearly an outlier that has a great impact on the fit.
...
Moreover, looking to the diagnostic plots of `nihill.lm2` we see that `Slieve Donard` and `Meelbeg Meelmore`  also have high residuals. 

Thus, we decide to remove all of these suspected points:
```{r}
library(dplyr) # for using %>% operator
nihills <- 
  list("Seven Sevens", "Annalong Horseshoe", "Slieve Donard", "Meelbeg Meelmore", "Flagstaff to Carling") %>% 
  subset(nihills, !(rownames(nihills) %in% .))
```

Without these points we do the two fits another time and we are going to see if the $F$-test is still rejected:
...
Without the removed points now it looks that both model don't have points with high residual or leverage. 

Moreover, the $F$-test on the new fitted models shows that there is no significant improvement in the more complex model over the simple one. 
...
We can observe that our boostrap-confidence interval results to be quite similar for both models. 
Moreover, these intervals results to be similar also to the built-in function provided by `predict.lm` method.
...
```
From this graph we can see that the nature of the relationship between `mpg` and `displacement` is different for transmission of type $0$ than it is for transmission of type $1$. However, a linear model seems to be reasonable for both groups, suggesting the nonlinearity observed from the plot in point $a$ is due to the fact that no distinction has been made between the two types of transmission.  
...
```
With `x2 <- rbinom(10, 1, x1)`, in the linear model $y = \beta_0+\beta_2x_2$ the $x_2$ predictor has a positive effect on the response variable (before it had a negative effect, i.e. -1.03) and the intercept has still a positive effect, but lower (from $3.27$ to $1.06$). As regards the model $y = \beta_0+\beta_1x_1+\beta_2x_2$ there is no significant difference than the former situation.
...
coef(y.lm2)
```
Finally, for `x2 <- rbinom(10, 1, .5)` we have that the coefficient is independent w.r.t. `x1`. We can notice that the `x2` predictor has positive effect and the second model still behaves in a similar way to the previous ones. However, the coefficient values have decreased compared to those obtained using `x2 <- rbinom(10, 1, x1)`; hence, the effect of the predictors $x_1$ and $x_2$ is reduced.
...
calls.glm <- glm(p ~ conc, family = binomial, weights = tot)
summary(calls.glm)
```
...
summary(model)
```
In order to desume a decision rule for use of CT using a risk threshold of $p=0.025$, we can consider the logit of p $log\bigg(\frac{p}{1-p}\bigg)=-3.66$ which is given by some combinations of predictors. Since the estimated intercept is equal to $-4.4889$ and keeping in mind that our predictors assume $1$ value if a certain clinical characteristic is present and $0$ value otherwise, patients will be sent for CT if they have a combination of clinical conditions that returns a value greater or equal to $-3.66+4.4889=0.8289$.   
...
The dispersion estimate was $2.69$. Using the quasipoisson family all the standard errors increase by a factor of $\sqrt{2.69}$ relative to the poisson family.
Standard errors and p-values taken from a model that assumed Poisson errors would be highly misleading because even if they are low, the model takes into into account assumptions on the data that are not true, like that the dispersion parameter is equal to 1.
...
simulated <- poissonsim(x , a = 2, b = -4, seed=123)
...
```{r, echo=TRUE}
simulated2 <- poissonsim(x, a = estimated_coeff[1], b = estimated_coeff[2], seed = 123)
mean(simulated$y == simulated2$y) # y is the Poisson response vector
```
The predictions of the estimated model are the same as those of the true model in 83% of cases.  

**(b)**  
```{r, echo=TRUE}
x <- seq(0, 0.99, 0.01)
b <- rnorm(1,4,5)
simulated <-  poissonsim(x, a = 2, b = -b, slope.sd = 5, seed = 173)
model <- glm(y ~ x, family = poisson, data = simulated)
model2 <- glm(y ~ x, family = quasipoisson, data = simulated)
summary(model)
summary(model2)
```
From the `summary` output we can see that the estimated coefficients are the same for both models. However, in the quasiPoisson model the dispersion parameter is much larger than that of the Poisson model. Thus, we have different standard errors and p-values. Indeed, in the Poisson model both `intercept` and `x` are highly significant, while this is no longer true in the quasiPoisson model.

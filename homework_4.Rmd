...
The result of the $F$ test gives raise to a p-value, which is the probability that the $F$ statistics is grater than the one observed assuming the null hypotesis $H_0: \beta_{\text{dist x climb}}=0$, is quite low. Thus we can reject the null hypotesis and therefore say that `nihills2.lm`, the most complex model that includes an interaction term, appears significantly better than Model 1 for `nihills` data. 
...
In fact, the `Residual vs Leverage` plot suggests that this point has a very large leverage and, at the same time, a high standardized residual. We think then `Seven Sevens` is clearly an outlier that has a great impact on the fit.
...
Without the removed points now it looks that both models don't have problematic points. However, `Slieve Bearnagh` stands outside the Cook's distance lines and therefore it can be an influential point.
...
The previous tentative choice was a mistake driven by outliers. Without them, there's no statistical justification to prefer one model respect to the other.
...
Moreover, these intervals are similar also to the ones of the built-in function provided by `predict.lm` method.
